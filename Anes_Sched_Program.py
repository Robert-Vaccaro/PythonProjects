...
def patLast(self):
    root3.deiconify()
    patFir_ent2 = enter1(self)
    phone_ent2 = enter1(self)
    patlab1 = Label(root3, text = "Last Patient Entered: ")
    patlab1.grid(row=1, column=0)
    patlab2 = Label(root3, text = patFir_ent2+', '+phone_ent2)
    root3.mainloop()
def aClick(self):
    global rowz
...
worksheet.add_table(14, 0, 199 , 16)
...
worksheet.add_table("A15:Q30", {'columns': [{'header': 'Site'},
...
subMenu.add_command(label="Last Patient Entered", command= patLast)
...
site_ent.focus()
pat_labels=Label(root, text='Last Patient: ')
pat_labels.grid(row=17,column=0)
...
root3.bind('<Control-Shift-X>',patLast)
